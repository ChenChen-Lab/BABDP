...
if ($fqlist && $host eq "human" && $annotation eq "vfdb_resfinder"){
	...
	(-d "$annotation_03/vfdb") ||`mkdir "$annotation_03/vfdb"`;
	(-d "$annotation_03/resfinder") ||`mkdir "$annotation_03/resfinder"`;
	open (LIST,">$annotation_03/genomes.list") ||die;
	open (IN,$fqlist)||die;
	while(my $line=<IN>){
		chomp($line);
		$sample=(split/\t/,$line)[0];
		...
		my $cmd_0="$fastqc --extract $fq1 -o $fastqc_00/result\n$fastqc --extract $fq2 -o $fastqc_00/result";
                print OUT0 $cmd_0."\n";
	...
	print OUT $cmd. "### step3_annotation\ncd $shelldir\n",
          ...
          "nohup sh $shelldir/step3_annotation.sh >$shelldir/step3_annotation.log &\n",
